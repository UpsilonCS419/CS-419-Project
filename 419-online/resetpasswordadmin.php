...
$checkreset = mysql_query("SELECT * FROM admin WHERE email='$useremail'") or die('No Such Email' . mysql_error());
...
<body>
<h1>Admin, Type your new password: <?php echo $_GET['email']; ?><h1>
...
{ 
	...
	if($resetpassword == $confirm){
		...
		$userrandomID = $_GET['reset'];
			...
			$results = mysql_query("SELECT * FROM admin WHERE email='$useremail' AND reset='$userrandomID'") or die('No Such Email' . mysql_error());
			$rows = mysql_fetch_array($results);
			$recovername = $rows['email'];
			$recoverID = $rows['id'];
			$randomID = (rand(10,10000));
			$sql = "UPDATE admin SET password='$resetpassword', reset='$randomID' where id='$recoverID'";
