...
{ 
	...
	$results = mysql_query("SELECT * FROM user WHERE email='$mail'") or die('No Such Email' . mysql_error());
	$rows = mysql_fetch_array($results);
	$recovername = $rows['email'];
	$recoverID = $rows['id'];
	$randomID = (rand(10,10000));
	$sql = "UPDATE user SET reset='$randomID' where id='$recoverID'";
...
if (!$result) 
{
  ...
  $to=$rows['email'];
  $resetURL = 'https://web.engr.oregonstate.edu/~hengs/wiki/docs/CS419FinalProject/resetpassworduser.php?email=' . $to . '&reset=' . $randomID;
